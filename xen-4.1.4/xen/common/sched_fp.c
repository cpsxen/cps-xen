...
static inline void print_vcpu(struct fp_vcpu *fpv)
{
	PRINT(2,"c.d.v:%d.%d.%d, state: %d, p: %d, idle: %d, time: %ld \n",
			...
			fpv->vcpu->runstate.state, fpv->priority, is_idle_vcpu(fpv->vcpu), (long int) NOW());
}
...
static void fp_stats(struct fp_vcpu * fpv, s_time_t now) {
...
static inline int __vcpu_on_queue(struct fp_vcpu *fpv, struct list_head *list)
{
	struct list_head *iter;
...
{
	if(!is_idle_vcpu(fpv->vcpu)) list_del_init(&fpv->queue_elem);
}
...
static inline void __runq_insert(unsigned int cpu, struct fp_vcpu *fpv, int (*compare)(struct fp_vcpu*, struct fp_vcpu*))
{
    ...
    if(is_idle_vcpu(fpv->vcpu)) return;
...
static int fp_sched_set(const struct scheduler *ops, struct xen_sysctl_fp_schedule *schedule)
{
    struct fpsched_private *prv = FPSCHED_PRIV(ops);
...
static void * fp_alloc_vdata(const struct scheduler *ops, struct vcpu *vc, void *dd)
{
    struct fp_vcpu *fpv;
...
static int fp_adjust_global(const struct scheduler *ops, struct xen_sysctl_scheduler_op *sc)
{
...
static void fp_sleep(const struct scheduler *ops, struct vcpu *vc)
{
    ...
    else if (__newvcpu_on_q(fpv) ) __runq_remove(fpv);
}
...
static int fp_adjust(const struct scheduler *ops, struct domain *d, struct xen_domctl_scheduler_op *op)
{
	...
	fp_sched_set_vm_prio(ops, d, op->u.fp.priority > 0 ? op->u.fp.priority : fp_dom->priority);
...
static struct task_slice fp_do_schedule(const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
	...
	if(!list_empty(runq))
	{
		list_for_each(iter, runq)
				 {
				 const struct fp_vcpu * const iter_fpv = __runq_elem(iter);
				 if (vcpu_runnable(iter_fpv->vcpu) && iter_fpv->awake && (iter_fpv->cputime < iter_fpv->slice))
				     {
	...
	if(snext== NULL) snext = FPSCHED_VCPU(idle_vcpu[cpu]);
