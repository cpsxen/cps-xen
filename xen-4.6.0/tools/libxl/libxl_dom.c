...
static void libxl__remus_domain_suspend_callback(void *data)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:libxl__remus_domain_suspend_callback");
                                ...
                                libxl__domain_suspend_state *dss, int rc)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:remus_domain_suspend_callback_common_done");
...
static void libxl__remus_domain_resume_callback(void *data)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:libxl__remus_domain_resume_callback");
                                       ...
                                       int rc)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:remus_devices_preresume_cb");
...
static void libxl__remus_domain_save_checkpoint_callback(void *data)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:libxl__remus_domain_save_checkpoint_callback");
    ...
    libxl__egc *egc, libxl__stream_write_state *sws, int rc)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:remus_checkpoint_stream_written");
    if (rc) {
                                    ...
                                    int rc)
{
    ...
    STATE_AO_GC(dss->ao);
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:remus_devices_commit_cb");
    ...
    if (libxl_defbool_val(dss->remus->event_driven) && !libxl_defbool_val(dss->remus->polling)) {
        /* Use event-driven checkpointing */
        if (!libxl__ev_xswatch_isregistered(&dss->cpsremus_watch)) {
            printf("Registerung xswatch on statepath: %s\n", dss->statepath);
    ...
    if (libxl_defbool_val(dss->remus->polling)) {
        ...
        if (!libxl__xs_transaction_start(gc, &t)) {
            ...
            if (state > 0) {
                ...
                libxl__ev_time_register_rel(ao, &dss->checkpoint_timeout,
                                         remus_next_checkpoint,
                                         dss->interval);
            }
                                  ...
                                  int rc)
{
    ...
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:libxl__remus_teardown");
                                       ...
                                       int rc)
{
    libxl__domain_suspend_state *dss = CONTAINER_OF(rds, *dss, rds);
    STATE_AO_GC(dss->ao);
    LIBXL__CPSREMUS_DBG_PRINT("libxl_dom.c:remus_teardown_done");
